...
dfacc = pd.read_csv(r"C:\Users\YASH KATTIMANI\Downloads\acc.csv")
# %%
...
df['CFV1_Proportion'] = df['CFV1'].map(lambda x: round((cfv1_value_counts[x] / len(df)) * 100, 2))
...
# Create a new column with proportions
df['CFV1_Proportion'] = df['CFV1'].map(lambda x: round((cfv1_value_counts[x] / len(df[df['CFV1'] != 'Unspecified'])) * 100, 2))
...
#%%
# Perform value counts on 'CFV3' column excluding 'Unspecified'
cfv3_value_counts = df[df['CFV3'] != 'Unspecified']['CFV3'].value_counts()

# Create a new column with proportions for 'CFV3'
df['CFV3_Proportion'] = df['CFV3'].map(lambda x: round((cfv3_value_counts[x] / len(df[df['CFV3'] != 'Unspecified'])) * 100, 2))
...
df['CFV4_Proportion'] = df['CFV4'].map(lambda x: round((cfv4_value_counts[x] / len(df[df['CFV4'] != 'Unspecified'])) * 100, 2))
